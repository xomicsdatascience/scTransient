...
def reconstruct_wavelet(scale: float,
    ...
    """
    Constructs a wavelet from the specified scale, position, and wavelet.
    Parameters
    ----------
    scale : float
        Scale of the wavlet
    ...
    -------
    np.array
        X-positions for the wavelet.
    np.array
        The signal containing the wavelet with specified parameters.
    """
...
def find_local_extrema(arr: np.ndarray,
    ...
    """
    Find local extrema in a 2D array. A local extremum is defined as a connected group (8-connected)
    ...
    # Define neighbours relative positions for 8-connectedness
...
def mag_median(coefs, get_st: bool = False):
    """
    Deprecated; use scTransient.metrics.transient_event_score instead.
    Parameters
    ----------
    coefs
    get_st
...
def link_two_pt_paths(df1,
    ...
    """Link two pseudotimeseries together."""
